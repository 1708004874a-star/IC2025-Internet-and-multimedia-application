...
void LedMatrix::drawSmoothText(int offset) {
    clear();
    int totalWidth = myText.length() * myCharWidth;

    for (int i = 0; i < myText.length(); i++) {
        char letter = myText.charAt(i);
        for (int col = 0; col < myCharWidth; col++) {
            int position = i * myCharWidth + col - offset;
            if (position >= 0 && position < myNumberOfDevices * 8) {
                byte columnData = pgm_read_byte(&cp437_font[letter][col]);
                setColumn(position, columnData);
            }
        }
    }
}
